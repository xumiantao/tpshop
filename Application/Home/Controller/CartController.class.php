...
//购物车
     public function shopping (){     
        ...
        $ishas=D('Cart')->where(array('user_id'=>session('user_id'),'isshoucan'=>2))->find();
            if(empty($ishas)){
                        $this->display('shopping');
                        // $this->redirect('Home/Goods/keep');
            }else{
            $cart=D('Goods')->join('tp_cart on tp_cart.goods_id=tp_goods.goods_id')->where(array('user_id'=>session('user_id'),'isshoucan'=>2))->select();
...
public function deleteshopping(){
    D('Cart')->where(array('user_id'=>session('user_id'),'isshoucan'=>1))->delete();
    $this->redirect('Home/index/all');
}  
...
public function established(){
    $cartaddress =D('UserAddress')->where(array('user_id'=>session('user_id'),'is_default'=>1))->find();
    $cart=D('Goods')->join('tp_cart on tp_cart.goods_id=tp_goods.goods_id')->where(array('user_id'=>session('user_id'),'isshoucan'=>2))->select();
    foreach ($cart as $key => $value) {
        if(!in_array($value['id'],$_POST['cart'])){
            unset($cart[$key]);
        }
    }
    foreach ($cart as $k => $v) {
        $cart[$k]['num']=$_POST['num'][$k];
    }
    $this->assign('price',$_POST['allprice']);
    $this->assign('ad',$cartaddress);
    $this->assign('cart',$cart);
    $this->display();
}
//加入购物车
public function addshop(){
    $sf=D('Goods')->where(array('goods_id'=>$_GET['goods_id']))->find();
    $fe['goods_name']=$sf['goods_name'];
    $fe['market_price']=$sf['market_price'];
    $fe['goods_id']=$sf['goods_id'];
    $fe['isshoucan']='2';
    $fe['user_id']=session('user_id');
    $ishas=D('Cart')->where(array('user_id'=>session('user_id'),'goods_id'=>$_GET['goods_id']))->find();
    if(!empty($ishas)){
        $this->redirect('Home/Cart/shopping');
        exit();
    }
    D('Cart')->add($fe);
    $this->redirect('Home/Cart/shopping');
...
public function pay(){
    ...
    $data['pay_status']=0;
    $data['consignee']=trim(I('post.get_name'));
    $data['address']=trim(I('post.address'));
    $data['mobile']=trim(I('post.phone'));
    $data['pay_name']="微信支付";
    $data['goods_price']=trim(I('post.payprice'));
    $data['order_amount']=trim(I('post.payprice'));
    $data['total_amount']=trim(I('post.payprice'));
    $data['add_time']=time();
    $data['user_note']=trim(I('post.saydetail'));
    $order_id=D('Order')->add($data);
    foreach ($_POST['goods_id'] as $key => $value) {
        $ordata['goods_id']=$value;
        $ordata['goods_num']=$_POST['goods_num'][$key];
        $fe=D('Goods')->where(array('goods_id'=>$value))->find();
        $ordata['market_price']=$fe['market_price'];
        $ordata['goods_price']=$fe['shop_price'];
        $ordata['is_send']=0;
        $ordata['order_id']=$order_id;
        $ordata['delivery_id']= $data['order_sn'];
        D('OrderGoods')->add($ordata);
    }
        //删除购物车数据
        foreach ($_POST['goods_id'] as $key => $value) {
            D('Cart')->where(array('user_id'=>session('user_id'),'goods_id'=>$value))->delete();            
        }
    ...
    {
        if(D('Cart')->where(array('user_id'=>session('user_id')))->delete()){
            $this->redirect('Home/Cart/shopping');
        }
    }
    /**
